...
export function BlueprintModal({
  ...
  const handleSave = async () => {
    if (!name.trim()) return showError(new Error("Blueprint name is required"))
    if (sections.length === 0)
      return showError(new Error("Add at least one section"))
    ...
    >
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className="fixed inset-x-0 bottom-0 z-[100] flex max-h-[90dvh] w-full flex-col overflow-hidden rounded-t-2xl bg-background shadow-2xl sm:inset-auto sm:top-1/2 sm:left-1/2 sm:max-w-lg sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-2xl"
